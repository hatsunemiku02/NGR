...
*/
void
bbox::get_clipplanes(const matrix44& viewProj, std::vector<plane>& outPlanes) const
{
    ...
    for (i = 0; i < 6; i++)
    {
        outPlanes.push_back(matrix44::transform(inverseTranspose,planes[i]));
    }
}
...
#if RENDERDEVICE_OPENGL || RENDERDEVICE_OPENGLES20
		 if (p1.z() < -p1.w())   clip |= ClipNear;//if (p1.z() < -1.0f)   clip |= ClipNear;
#endif
