...
public:
	...
	static float3 lerp(const float3 &v0, const float3 &v1, scalar s);
	/// convert to anything
...
float3::lerp(const float3 &v0, const float3 &v1, scalar s)
{
	float3 Result;

	Result.x() = (v0.x() + s * (v1.x() - v0.x()));
	Result.y() = (v0.y() + s * (v1.y() - v0.y()));
	Result.z() = (v0.z() + s * (v1.z() - v0.z()));
